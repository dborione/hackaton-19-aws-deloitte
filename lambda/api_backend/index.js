const sharing = require("./sharingHandlers");
...
exports.handler = async (event) => {
  const sharingRouteResponse = await sharing.tryHandleSharingRoute(event);
  if (sharingRouteResponse) {
    return sharingRouteResponse;
  }
  ...
  // GET /data/answers
  if (method === "GET" && path === "/data/answers") {
    const userId = getUserId(event);

    const result = await rds.send(new ExecuteStatementCommand({
      ...DB_PARAMS,
      sql: `
        SELECT page_id, answers
        FROM user_answers
    ...
    for (const row of result.records || []) {
      const pageId = row[0].stringValue;
      const answers = JSON.parse(row[1].stringValue || "{}");

      answersByPage[pageId] = answers;
    }
